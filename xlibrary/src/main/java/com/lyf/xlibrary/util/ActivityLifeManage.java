 ...
 * 公司：
 */
public class ActivityLifeManage {
    private static final String TAG = "ActivityLifeManage";
    private static final Object LOCK = new Object();
    private volatile static ActivityLifeManage mManager = null;
    private Context mContext;
    private Stack<Activity> mActivityStack = new Stack<>();

    private ActivityLifeManage() {

    }

    private ActivityLifeManage(Context context) {
        mContext = context;
    }
     ...
     * @return
     */
    public static ActivityLifeManage init(Context applicationContext) {
        if (mManager == null) {
            synchronized (LOCK) {
                if (mManager == null) {
                    mManager = new ActivityLifeManage(applicationContext);
                }
            }
        }
        return mManager;
    ...
    /**
     * 开启异常捕获
     * 日志文件位于/data/data/<Package Name>/cache//crash/ExceptionFile.crash
     */
     ...
     * @param key        数据传输键值
     */
    public ActivityLifeManage openCrashHandler(String serverHost, String key) {
